-- =============================================================================
-- Portal publication lifecycle
--
-- Keeps page + visible section versions in one database transaction. The
-- application cannot safely implement publication as a sequence of unrelated
-- client updates because a failure halfway through would leave the public
-- portal in a mixed state.
-- =============================================================================
...
begin
  ...
  -- A page can only be reviewed once every visible section has a current
  -- version. Hidden sections may remain drafts and are never exposed.
...
is 'Atomically transitions a portal page and its visible current section versions through the publication lifecycle. Permission is checked inside the SECURITY DEFINER function.';
