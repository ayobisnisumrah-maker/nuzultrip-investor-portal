-- Repair production portal state after an older transition function cleared
-- published_at and left section containers in draft while a published snapshot
-- still existed.

begin;

-- Recover the public snapshot marker only for editorial pages that demonstrably
-- still have published section versions.
with recovered as (
  select
    p.id,
    coalesce(max(v.published_at), max(v.created_at), now()) as published_at
  from public.portal_pages p
  join public.portal_sections s on s.page_id = p.id
  join public.portal_section_versions v on v.id = s.published_version_id
  where p.status in ('draft', 'review', 'approved')
    and p.published_at is null
    and s.published_version_id is not null
  group by p.id
)
update public.portal_pages p
set published_at = recovered.published_at
from recovered
where p.id = recovered.id;

-- While a revision is in progress, section.status describes the public snapshot.
-- Restore it for sections that still have a valid published_version_id.
update public.portal_sections s
set status = 'published'
from public.portal_pages p
where p.id = s.page_id
  and p.status in ('draft', 'review', 'approved')
...
begin
  ...
  insert into public.portal_section_versions (
    section_id, version_number, status, content, change_note, created_by
  ) values (
    old.id,
    v_next_version,
    'draft',
...
begin
  ...
  if p_to_status = 'review' then
    ...
    from public.portal_sections s
    where s.page_id = v_page.id and s.current_version_id is null;
