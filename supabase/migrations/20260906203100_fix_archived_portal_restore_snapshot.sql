-- Preserve immutable published history when an archived portal page is restored.
-- The previous remediation cleared published_version_id on archived -> draft,
-- which broke the established lifecycle contract and its regression test.
...
begin
  ...
  if p_to_status = 'review' then
    ...
    if v_page.status = 'archived' then
      -- Create fresh editable copies from the last published snapshot while
      -- preserving published_version_id as immutable publication history.
      with copied_versions as (
        ...
        join public.portal_section_versions v
          on v.id = coalesce(s.published_version_id, s.current_version_id)
        where s.page_id = v_page.id
      ...
      where s.id = copied_versions.section_id;
