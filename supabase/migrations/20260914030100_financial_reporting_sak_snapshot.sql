-- Version-bound accounting framework, basis of preparation, statement of
-- changes in equity, and notes to the financial statements (CALK).
-- No framework is selected automatically and this schema does not claim
...
comment on column public.financial_report_versions.accounting_framework is
  'Framework explicitly selected by management; null means not declared and never implies compliance.';
comment on column public.financial_report_versions.basis_of_preparation is
  'Version-bound basis of preparation disclosed with the financial statements.';

create table public.financial_report_disclosures (
  ...
  constraint financial_report_disclosures_key_shape check (disclosure_key ~ '^[a-z][a-z0-9_]*$'),
  constraint financial_report_disclosures_title_not_blank check (length(btrim(title)) > 0),
  constraint financial_report_disclosures_content_not_blank check (length(btrim(content)) > 0),
  constraint financial_report_disclosures_unique unique (financial_report_version_id, disclosure_key)
);

create index financial_report_disclosures_version_idx
...
comment on table public.financial_report_disclosures is
  'Version-bound notes to the financial statements (CALK). Factual content is entered/reviewed by management or accounting personnel and is never fabricated by the system.';
...
begin
  ...
  select
    ...
    btrim(item.line_key), btrim(item.label), item.amount,
    upper(btrim(item.currency)), item.position,
    nullif(btrim(coalesce(item.note, '')), '')
  from jsonb_to_recordset(coalesce(p_line_items, '[]'::jsonb)) as item(
    statement text, category text, line_key text, label text,
    amount numeric, currency text, position integer, note text
  );
  ...
  select
    v_version.id, btrim(item.kpi_key), btrim(item.label), item.value,
    btrim(item.unit), btrim(item.basis), item.position
  from jsonb_to_recordset(coalesce(p_kpis, '[]'::jsonb)) as item(
    kpi_key text, label text, value numeric, unit text, basis text, position integer
  );
  ...
  select
    v_version.id, btrim(item.disclosure_key), btrim(item.title), btrim(item.content), item.position
  from jsonb_to_recordset(coalesce(p_disclosures, '[]'::jsonb)) as item(
    disclosure_key text, title text, content text, position integer
  );
  ...
  if v_report.status = 'draft' and p_target = 'review' then
    ...
    if v_version.accounting_framework is not null then
      if nullif(btrim(coalesce(v_version.basis_of_preparation, '')), '') is null then
        raise exception 'Add the basis of preparation before reviewing a framework-declared report.' using errcode = '23514';
      end if;
      if not exists (
        select 1 from public.financial_line_items
        where financial_report_version_id = v_version.id and statement = 'changes_in_equity'
      ) then
        raise exception 'Add the statement of changes in equity before reviewing a framework-declared report.' using errcode = '23514';
      end if;
      if not exists (
        select 1 from public.financial_report_disclosures
        where financial_report_version_id = v_version.id
      ) then
        raise exception 'Add notes to the financial statements before reviewing a framework-declared report.' using errcode = '23514';
      end if;
    end if;
    ...
    if v_period_status not in ('closed', 'locked') then
      raise exception 'Financial period must be closed or locked before publishing the report.' using errcode = '23514';
    end if;
    if v_period_end > current_date then
      raise exception 'Financial report cannot be published before the period end date (%).', v_period_end using errcode = '23514';
    end if;
  else
