...
const createSchema = z.object({
  ...
  revenueAmount: z.number().finite().nonnegative(),
  opexAmount: z.number().finite().nonnegative(),
...
export const createProfitDistributionAction = defineAction({
  ...
  handler: async ({ input, supabase, audit }) => {
    const { data, error } = await appRpcClient(supabase).rpc('create_profit_distribution', {
      ...
      p_opex_amount: input.opexAmount,
    ...
    if (!distributionId) throw new ConflictError('Distribution RPC returned no id.', 'Distribusi bagi hasil tidak dapat dibuat saat ini.')
    audit({
...
export const regenerateProfitDistributionAllocationsAction = defineAction({
  ...
  audit: { action: 'profit_distribution.allocations_regenerated', entityType: 'profit_distribution' },
  handler: async ({ input, supabase, audit }) => {
    const { data, error } = await appRpcClient(supabase).rpc('regenerate_profit_distribution_allocations', {
      p_distribution_id: input.distributionId,
    })
    if (error) throw new ConflictError(`Failed to regenerate allocations: ${error.message}`, 'Allocation investor tidak dapat dihitung saat ini.')
    const count = Array.isArray(data) ? data.length : data ? 1 : 0
    audit({ entityId: input.distributionId, summary: `${count} allocation investor dihitung ulang.` })
    revalidatePath('/admin/profit-distributions')
...
function transition(target: 'review' | 'approved' | 'payable', permission: 'profit_distributions.update' | 'profit_distributions.approve' | 'profit_distributions.publish') {
  return defineAction({
    access: { permission },
    input: distributionSchema,
    ...
    handler: async ({ input, supabase, audit }) => {
      ...
      if (error) throw new ConflictError(`Failed to transition distribution: ${error.message}`, 'Status distribusi bagi hasil tidak dapat diperbarui saat ini.')
      const row = rowFrom(data)
      audit({ entityId: input.distributionId, summary: `Distribusi bagi hasil berubah menjadi ${target}.` })
      revalidatePath('/admin/profit-distributions')
      revalidatePath(`/admin/profit-distributions/${input.distributionId}`)
      revalidatePath('/investor')
      revalidatePath('/investor/distributions')
      return { distributionId: input.distributionId, status: typeof row?.status === 'string' ? row.status : target }
    },
  })
}

export const submitProfitDistributionForReviewAction = transition('review', 'profit_distributions.update')
export const approveProfitDistributionAction = transition('approved', 'profit_distributions.approve')
