...
export const markProfitDistributionAllocationPaidAction =
  defineAction({
    access: {
    ...
    handler: async ({
      ...
      const allocation =
        await markProfitDistributionAllocationPaid(
          supabase,
          input.allocationId,
          paymentReference,
        )
      ...
      audit({
        ...
        summary:
          'Pembayaran distribusi bagi hasil ditandai sebagai paid.',
        changes: {
          status: {
            before: 'payable',
            after: 'paid',
      ...
      const distribution =
        await refreshProfitDistributionPaidStatus(
          supabase,
