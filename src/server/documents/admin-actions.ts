...
export const grantDocumentAccess = defineAction({
  ...
  handler: async ({ input, supabase, audit, principal }) => {
    ...
    const { data: grant, error } = await supabase
      .from('document_access_grants')
      .insert({ document_id: document.id, investor_id: investor.id, note: input.note || null })
...
export const revokeDocumentAccess = defineAction({
  ...
  handler: async ({ input, supabase, audit, principal }) => {
    ...
    const { data: grant, error: readError } = await supabase
      .from('document_access_grants')
      .select(
        'id, document_id, investor_id, revoked_at, documents(title), investors(reference_code)',
      )
      .eq('id', input.grantId)
      .maybeSingle()
    if (readError)
