...
type RevokeGrantInvestor = { reference_code: string }
type RevokeGrantRow = {
  id: string
  document_id: string
  investor_id: string
...
export const createDocument = defineAction({
  ...
  audit: { action: 'document.created', entityType: 'document' },
  handler: async ({ input, supabase, audit }) => {
    const { data, error } = await supabase.schema('app').rpc('create_document_with_draft', {
      p_title: input.title,
      p_slug: input.slug,
      p_kind: input.kind,
...
function defineTransitionAction(
  target: PublicationStatus,
  permission: 'documents.review' | 'documents.approve' | 'documents.publish' | 'documents.archive',
  action: string,
...
export const submitDocumentForReview = defineTransitionAction(
  'review',
  'documents.review',
  'document.review_started',
  (title) => `Dokumen ${title} dikirim untuk peninjauan.`,
)
export const approveDocument = defineTransitionAction(
  'approved',
  'documents.approve',
  'document.approved',
  (title) => `Dokumen ${title} disetujui.`,
)
export const publishDocument = defineTransitionAction(
  'published',
  'documents.publish',
  'document.published',
  (title) => `Dokumen ${title} diterbitkan.`,
)
export const archiveDocument = defineTransitionAction(
  'archived',
  'documents.archive',
  'document.archived',
  (title) => `Dokumen ${title} diarsipkan.`,
)

export const grantDocumentAccess = defineAction({
  access: { permission: 'investor_documents.assign' },
  input: grantSchema,
  audit: { action: 'document.access_granted', entityType: 'document' },
  handler: async ({ input, supabase, audit, principal }) => {
    const admin = requirePermission(principal, 'investor_documents.assign')
    ...
    if (investor.status !== 'approved' && investor.status !== 'active') {
      throw new ConflictError(
        `Investor status is ${investor.status}.`,
        'Akses hanya dapat diberikan kepada investor yang telah disetujui atau aktif.',
      )
    }
    ...
    if (existingError) {
      throw new ConflictError(
        `Failed to read document access grant: ${existingError.message}`,
        'Akses dokumen tidak dapat diperiksa saat ini.',
      )
    }
    if (existing) {
      throw new ConflictError(
        'Live document grant already exists.',
        'Investor sudah memiliki akses ke dokumen ini.',
      )
    }
    ...
    if (error || !grant) {
      throw new ConflictError(
        `Failed to grant document access: ${error?.message ?? 'no row'}`,
        'Akses dokumen tidak dapat diberikan saat ini.',
      )
    }

    audit({
      entityId: document.id,
      summary: `Akses dokumen ${document.title} diberikan kepada investor ${investor.reference_code}.`,
      changes: { access: { before: null, after: { investorId: investor.id, grantId: grant.id } } },
    })
...
export const revokeDocumentAccess = defineAction({
  access: { permission: 'investor_documents.revoke' },
  input: revokeSchema,
  audit: { action: 'document.access_revoked', entityType: 'document' },
  handler: async ({ input, supabase, audit, principal }) => {
    const admin = requirePermission(principal, 'investor_documents.revoke')
    requirePermission(principal, 'investor_documents.view')

    const { data: grant, error: readError } = await supabase
      .from('document_access_grants')
      .select('id, document_id, investor_id, revoked_at, documents(title), investors(reference_code)')
      .eq('id', input.grantId)
      .maybeSingle()
    if (readError) {
      throw new ConflictError(
        `Failed to read document access grant: ${readError.message}`,
        'Akses dokumen tidak dapat dibaca saat ini.',
      )
    }
    if (!grant) throw new NotFoundError('Akses dokumen')
    if (grant.revoked_at) {
      throw new ConflictError('Document grant is already revoked.', 'Akses dokumen sudah dicabut.')
    }

    const { error } = await supabase
      .from('document_access_grants')
      .update({ revoked_at: new Date().toISOString(), revoked_by: admin.adminId })
      .eq('id', grant.id)
      .is('revoked_at', null)
    if (error) {
      throw new ConflictError(
        `Failed to revoke document access: ${error.message}`,
        'Akses dokumen tidak dapat dicabut saat ini.',
      )
    }

    const revokeGrant = grant as unknown as RevokeGrantRow
    const document = Array.isArray(revokeGrant.documents)
      ? revokeGrant.documents[0]
      : revokeGrant.documents
    const investor = Array.isArray(revokeGrant.investors)
      ? revokeGrant.investors[0]
      : revokeGrant.investors

    audit({
      entityId: grant.document_id,
      summary: `Akses dokumen ${document?.title ?? grant.document_id} dicabut dari investor ${investor?.reference_code ?? grant.investor_id}.`,
      changes: {
        access: { before: { investorId: grant.investor_id, grantId: grant.id }, after: null },
      },
    })
