...
function Atmosphere({ scrollY }: { scrollY: number }) {
  ...
  useEffect(() => {
    ...
    const colors = ['rgba(52, 211, 153, ', 'rgba(16, 185, 129, ', 'rgba(45, 212, 191, ', 'rgba(226, 232, 240, ']
  ...
  return <div className="absolute inset-0 pointer-events-none overflow-hidden z-0" aria-hidden="true">
    <div className="absolute inset-0 transition-opacity duration-300" style={{background:'radial-gradient(ellipse 130% 90% at 50% 15%, #182230 0%, #111822 55%, #0d141d 100%)'}}/>
    <div className="absolute w-[800px] h-[500px] -top-[120px] left-1/2 -translate-x-1/2 rounded-full blur-[130px] will-change-transform" style={{opacity:Math.max(.3,1-progress*.6),transform:`translate3d(-50%, ${arcY*.4}px, 0) scale(${1+progress*.15})`,background:'radial-gradient(50% 50% at 50% 50%, rgba(16, 185, 129, 0.14) 0%, rgba(13, 148, 136, 0.08) 50%, transparent 100%)'}}/>
...
export function V2Hero({ id, eyebrow, titleLines, description, primary, secondary }: HeroProps) {
  ...
  return <section id={id} className="relative min-h-[100dvh] flex flex-col justify-center items-center overflow-hidden bg-[#111822] text-white pt-24 pb-12 px-4 sm:px-6 lg:px-8">
    ...
    <div className="relative z-10 w-full max-w-5xl mx-auto flex flex-col items-center text-center my-auto space-y-6 sm:space-y-7">
      <div className={`${entrance} stagger-1 inline-flex items-center gap-2 px-3.5 py-1.5 rounded-full bg-emerald-950/60 border border-emerald-500/30 text-emerald-300 text-[11px] sm:text-[12px] font-bold tracking-[0.16em] uppercase shadow-xs`} style={heading}><span className="w-1.5 h-1.5 rounded-full bg-emerald-400 shadow-[0_0_8px_rgba(52,211,153,0.8)]"/><span>{eyebrow}</span></div>
      <h1 className={`${entrance} stagger-2 text-[32px] sm:text-[48px] lg:text-[62px] font-extrabold text-white tracking-tight leading-[1.08] max-w-4xl text-balance`} style={heading}>{titleLines.map((line,i)=>i===titleLines.length-1?<span key={line} className="bg-gradient-to-r from-emerald-400 via-teal-300 to-emerald-400 bg-clip-text text-transparent"> {line}</span>:<span key={line}>{line} </span>)}</h1>
      {description?<p className={`${entrance} stagger-3 text-[15px] sm:text-[17px] text-slate-300 max-w-2xl leading-relaxed text-balance`} style={subtitle}>{description}</p>:null}
      <div className={`${entrance} stagger-4 flex flex-col sm:flex-row items-center justify-center gap-3.5 sm:gap-4 w-full sm:w-auto pt-2`} style={buttons}><Link href={primary.href} className="w-full sm:w-auto px-8 py-3.5 sm:py-4 rounded-full font-bold text-sm sm:text-base text-[#0d151d] bg-gradient-to-r from-emerald-400 via-emerald-500 to-teal-500 hover:from-emerald-300 hover:to-teal-400 active:scale-98 transition-all duration-200 shadow-md shadow-emerald-600/25 flex items-center justify-center gap-2.5"><span>{primary.label}</span><ArrowRight size={17}/></Link><Link href={secondary.href} className="w-full sm:w-auto px-7 py-3.5 sm:py-4 rounded-full font-semibold text-sm sm:text-base text-slate-100 bg-white/10 hover:bg-white/15 border border-white/20 hover:border-white/30 backdrop-blur-md active:scale-98 transition-all duration-200 shadow-xs flex items-center justify-center gap-2.5"><Download size={16} className="text-emerald-400"/><span>{secondary.label}</span></Link></div>
      <div className={`${entrance} stagger-5 w-full max-w-4xl pt-6 sm:pt-8 border-t border-white/10 mt-4 sm:mt-6`} style={{transform:`translate3d(0, ${scrollY*.03}px, 0)`,opacity:Math.max(0,1-scrollY/420),transition:'transform 0.05s linear'}}><div className="text-[10.5px] sm:text-[11.5px] font-bold text-emerald-300 uppercase tracking-[0.18em] mb-3.5 text-center">Sorotan Ekosistem Investasi</div><div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-6 gap-2.5 sm:gap-3.5">{metrics.map(([value,label])=><div key={label} className="py-3 px-2 sm:px-3 rounded-xl bg-[#182332]/75 backdrop-blur-sm border border-white/10 hover:border-emerald-500/40 hover:bg-[#1e2b3e]/85 transition-all flex flex-col items-center justify-center text-center"><span className="text-[17px] sm:text-[19px] font-extrabold text-white tracking-tight leading-none mb-1">{value}</span><span className="text-[11px] text-slate-300 font-medium leading-tight">{label}</span></div>)}</div></div>
      <div className="flex flex-col items-center gap-1 pt-2 pointer-events-none select-none transition-opacity duration-300" style={{opacity:Math.max(0,1-scrollY/90)}} aria-hidden="true"><span className="text-[10px] tracking-[0.2em] uppercase font-bold text-slate-400">Scroll Eksplorasi</span><ChevronDown size={14} className="text-emerald-400 animate-bounce"/></div>
    </div>
  </section>
}
