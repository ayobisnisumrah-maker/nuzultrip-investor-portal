          ...
          <div className="border-border border-b p-4">
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="text-caption text-fg-muted mt-0.5">{threads.length} percakapan</p>
              </div>
              {canSend ? (
                <button
                  type="button"
                  ...
                  return (
                    ...
                    >
                      <div className="flex min-w-0 gap-3">
                        <div className={`flex size-10 shrink-0 items-center justify-center rounded-full text-xs font-bold ${
                          active ? 'bg-accent-solid text-on-accent' : 'bg-surface-muted text-fg-muted'
                        }`}>
                          {initials(investorName)}
                        </div>
                        <div className="min-w-0 flex-1">
                          <div className="flex min-w-0 items-start justify-between gap-2">
                            <p className="text-body-sm text-fg truncate font-semibold">{investorName}</p>
                ...
                <div className="min-w-0 flex-1">
                  <div className="flex min-w-0 items-center gap-2">
                    ...
                    {selectedThread.is_closed ? (
                      <span className="border-border text-caption text-fg-subtle shrink-0 rounded-full border px-2 py-0.5">Ditutup</span>
                    ) : null}
                  </div>
                  <p className="text-caption text-fg-muted mt-0.5 truncate">
                    {selectedThread.subject}
                    {selectedInvestor?.reference_code ? ` · ${selectedInvestor.reference_code}` : ''}
                  </p>
                </div>
              </div>
                ...
                {inquiries.length ? (
                  inquiries.map((inquiry) => (
                    <tr key={inquiry.id}>
                      ...
                      <td className="px-4 py-4">
                        <span className="border-border text-caption text-fg rounded-full border px-2.5 py-1">{inquiry.status}</span>
                      </td>
                  ...
                  <tr>
                    <td colSpan={5} className="text-body-sm text-fg-muted px-4 py-10 text-center">Belum ada permintaan masuk.</td>
                  </tr>
                )}
              </tbody>
            </table>
