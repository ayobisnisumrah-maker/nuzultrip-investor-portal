    ...
    <div className="grid gap-6">
      ...
      <form onSubmit={uploadIdentity} className="border-border grid gap-4 border-t pt-6">
        ...
        {investor.identityFileName ? (
          <p className="text-body-sm text-fg-muted">Tersimpan: <span className="font-medium text-fg">{investor.identityFileName}</span>{investor.identityUploadedAt ? ` · ${new Date(investor.identityUploadedAt).toLocaleString('id-ID')}` : ''}</p>
        ) : null}
        {documentMessage ? (
          <Alert tone={documentMessage.tone} title={documentMessage.tone === 'success' ? 'Berhasil' : 'Gagal mengunggah'}>{documentMessage.text}</Alert>
        ) : null}
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          ...
          <Button type="submit" variant="secondary" disabled={uploading}>
            {uploading ? 'Mengunggah…' : investor.identityFileName ? 'Ganti dokumen' : 'Unggah dokumen'}
          </Button>
        </div>
      </form>
    </div>
