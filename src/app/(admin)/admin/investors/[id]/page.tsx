...
export default async function AdminInvestorDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if (!principal.permissions.has('investors.view')) {
    return <Alert tone="info" title="Akses terbatas">Peran Anda tidak memiliki izin untuk melihat data investor.</Alert>
  }

  const supabase = await getServerSupabase()
  const { data: investor, error } = await supabase.from('investors').select('id, reference_code, status, investor_type, legal_name, organization_name, organization_role, country, city, address, application_note, rejection_reason, applied_at, reviewed_at, approved_at, activated_at, deactivated_at, created_at, updated_at').eq('id', id).maybeSingle()

  if (error) {
    return <Alert tone="danger" title="Data investor tidak dapat dimuat">Sistem gagal mengambil data investor. Silakan coba lagi.</Alert>
  }
  ...
  const [historyResult, auditResult, grantsResult] = await Promise.all([
    ...
    principal.permissions.has('investor_documents.view') ? supabase.from('document_access_grants').select('id, granted_at, note, documents(title, kind, visibility, status)').eq('investor_id', id).is('revoked_at', null).order('granted_at', { ascending: false }) : Promise.resolve({ data: null, error: null }),
  ])
  ...
  return (
    <Stack gap={8}>
      <RealtimeRefresher topic={topics.investor(id)} kinds={['investor.status_changed', 'investor.document_shared', 'investor.document_revoked']} />
      <PageHeader
        className="motion-safe:animate-rise"
        eyebrow="Investor Relations"
        title={name}
        description={<span className="flex flex-wrap items-center gap-x-3 gap-y-1"><span className="font-mono text-caption">{investor.reference_code}</span><span>{investor.investor_type === 'institution' ? 'Institusi' : 'Individu'}</span><span className="inline-flex items-center gap-1"><MapPin className="size-3.5" aria-hidden="true" />{location}</span></span>}
        actions={<Button asChild variant="secondary"><Link href="/admin/investors"><ArrowLeft aria-hidden="true" />Kembali ke Investor</Link></Button>}
      />
        ...
        <Stack gap={6}>
          <Card className="motion-safe:animate-rise">
            ...
            <CardBody><DetailList>
              ...
              {investor.organization_name ? <DetailRow label="Nama organisasi">{investor.organization_name}</DetailRow> : null}
              <DetailRow label="Tipe investor">{investor.investor_type === 'institution' ? 'Institusi' : 'Individu'}</DetailRow>
              {investor.organization_role ? <DetailRow label="Posisi organisasi">{investor.organization_role}</DetailRow> : null}
              <DetailRow label="Negara">{investor.country}</DetailRow><DetailRow label="Kota">{value(investor.city)}</DetailRow><DetailRow label="Alamat"><span className="whitespace-pre-wrap">{value(investor.address)}</span></DetailRow>
              {investor.application_note ? <DetailRow label="Catatan pengajuan"><span className="whitespace-pre-wrap">{investor.application_note}</span></DetailRow> : null}
              {investor.rejection_reason ? <DetailRow label="Alasan penolakan"><span className="whitespace-pre-wrap">{investor.rejection_reason}</span></DetailRow> : null}
              <DetailRow label="Tanggal pengajuan">{date(investor.applied_at)}</DetailRow><DetailRow label="Dibuat">{date(investor.created_at)}</DetailRow><DetailRow label="Diperbarui">{date(investor.updated_at)}</DetailRow><DetailRow label="Ditinjau">{date(investor.reviewed_at)}</DetailRow><DetailRow label="Disetujui">{date(investor.approved_at)}</DetailRow><DetailRow label="Diaktifkan">{date(investor.activated_at)}</DetailRow>{investor.deactivated_at ? <DetailRow label="Dinonaktifkan">{date(investor.deactivated_at)}</DetailRow> : null}
            </DetailList></CardBody>
          </Card>
