...
import { PUBLICATION_STATUSES, PUBLICATION_STATUS_LABELS, VISIBILITIES, VISIBILITY_LABELS } from '@/core/documents/publication'
...
export default async function AdminDocumentsPage({ searchParams }: { searchParams: Promise<{ q?: string; status?: string; kind?: string; visibility?: string; page?: string }> }) {
  ...
  const columns: Column<(typeof rows)[number]>[] = [{ id: 'title', header: 'Dokumen', primary: true, cell: (row) => <Link className="font-medium text-fg hover:underline" href={`/admin/documents/${row.id}`}>{row.title}</Link> }, { id: 'kind', header: 'Tipe', cell: (row) => kinds[row.kind] }, { id: 'status', header: 'Status', cell: (row) => <PublicationBadge status={row.status} /> }, { id: 'visibility', header: 'Visibilitas', cell: (row) => <VisibilityBadge visibility={row.visibility} /> }, { id: 'updated', header: 'Diperbarui', cell: (row) => formatDateTime(row.updated_at) }]
  return <Stack gap={8}><RealtimeRefresher topic={topics.admin()} kinds={['document.state_changed']} /><PageHeader
