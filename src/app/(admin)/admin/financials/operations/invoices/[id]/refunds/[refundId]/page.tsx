...
export default async function RefundRequestPage({
  ...
  const [refundResult, invoiceResult, itemsResult, paymentsResult] = await Promise.all([
    ...
    supabase
      ...
      .select(
        'id,reference,status,customer_name,customer_email,customer_phone,customer_address,grand_total,paid_total,refunded_total,currency,issued_on,due_on,company_snapshot',
      )
      .eq('id', id)
      .maybeSingle(),
    supabase
    ...
    <Stack gap={6} className="print:gap-0">
      ...
      <article className="bg-white text-black mx-auto w-full max-w-[210mm] min-h-[297mm] border border-black/15 p-[16mm] shadow-sm print:max-w-none print:min-h-0 print:border-0 print:p-[12mm] print:shadow-none">
        <header className="flex items-start justify-between gap-8 border-b border-black pb-6">
          <div className="min-w-0">
            {logoUrl ? (
              ...
              <img src={logoUrl} alt="Logo perusahaan" className="mb-4 h-14 max-w-48 object-contain object-left" />
            ) : null}
          ...
          <div className="text-right text-sm leading-6">
            <p className="font-semibold">{company.legalName || 'PT Swarna Dipa Wisata (Nuzultrip)'}</p>
        ...
        <section className="mt-8">
          <h2 className="border-b border-black pb-2 text-base font-bold">Data Jamaah / Pelanggan</h2>
        ...
        <section className="mt-8">
          <h2 className="border-b border-black pb-2 text-base font-bold">Data Paket / Transaksi</h2>
              ...
              <tbody>
                {items.map((item) => (
                  <tr key={item.id} className="border-b border-black/20 last:border-0">
                    ...
                    <td className="p-2">{Number(item.quantity)} {item.unit_label}</td>
        ...
        <section className="mt-8 grid grid-cols-2 gap-8 text-sm">
          <div>
            <h2 className="border-b border-black pb-2 text-base font-bold">Ringkasan Pembayaran Kasir</h2>
            <div className="mt-4 grid gap-3">
              ...
              <Data label="Referensi pembayaran" value={lastPayment?.external_reference || lastPayment?.reference || '—'} />
        ...
        <footer className="mt-12 border-t border-black pt-4 text-xs leading-5">
          <p>
            Form ini dibuat dari data transaksi kasir. Pengajuan berstatus requested/approved belum
            mengurangi laporan keuangan. Nilai refund masuk ke laporan hanya setelah status menjadi
            processed.
          </p>
