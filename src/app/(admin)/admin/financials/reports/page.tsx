...
export default async function FinancialReportsPage() {
  ...
  if (!principal) {
    return (
      <Alert tone="info" title="Akses terbatas">
        Peran Anda tidak memiliki izin untuk melihat laporan keuangan.
      </Alert>
    )
  }
  ...
  const { data: reports, error } = await supabase
    ...
    .select(
      'id, financial_period_id, title, summary, visibility, status, current_version_id, published_version_id, created_at, updated_at',
    )
    ...
    return (
      <Stack gap={6}>
        <PageHeader
          ...
          description="Kelola laporan keuangan dan lifecycle publikasinya untuk investor."
        />
        <Alert tone="danger" title="Laporan tidak dapat dimuat">
          Sistem gagal mengambil laporan keuangan. Silakan coba lagi.
        </Alert>
      </Stack>
    )
  }
  ...
  const { data: periods } = periodIds.length
    ? await supabase
        ...
        .select('id, period_type, fiscal_year, period_index, currency, starts_on, ends_on, status')
        .in('id', periodIds)
    : { data: [] }
    ...
    <Stack gap={8}>
      ...
      {!rows.length ? (
        <EmptyState
          ...
          description="Laporan yang dibuat dari periode keuangan akan muncul di halaman ini tanpa data contoh."
        />
      ) : (
                ...
                <CardHeader>
                  <div className="flex flex-wrap items-start justify-between gap-3">
                    <div>
                      ...
                      <p className="text-body-sm text-fg-muted mt-1">{formatPeriod(period)}</p>
                    </div>
                    <span className="border-border rounded-full border px-3 py-1 text-caption font-medium">
                      {PUBLICATION_STATUS_LABELS[report.status]}
                    </span>
                  </div>
                </CardHeader>
                <CardBody>
                  <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-4">
                    <div>
                      <div className="text-caption text-fg-subtle">Visibilitas</div>
                      <div className="mt-1 text-body-sm font-medium">{VISIBILITY_LABELS[report.visibility]}</div>
                    </div>
                    <div>
                      <div className="text-caption text-fg-subtle">Mata uang</div>
                      <div className="mt-1 text-body-sm font-medium">{period?.currency ?? '—'}</div>
                    </div>
                    <div>
                      <div className="text-caption text-fg-subtle">Versi terbit</div>
                      <div className="mt-1 text-body-sm font-medium">{report.published_version_id ? 'Tersedia' : 'Belum tersedia'}</div>
                    </div>
                    <div>
                      <div className="text-caption text-fg-subtle">Diperbarui</div>
                      <div className="mt-1 text-body-sm font-medium">{formatDate(report.updated_at)}</div>
                    </div>
                  </div>
