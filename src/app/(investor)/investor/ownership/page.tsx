  ...
  return (
    <Stack gap={8}>
      <PageHeader
        eyebrow="Ownership"
        title="Kepemilikan"
        description="Ringkasan unit dan porsi kepemilikan yang tercatat atas nama Anda."
      />
      {!holdings?.length ? (
        <EmptyState
          title="Belum ada kepemilikan"
          description="Holding yang telah dicatat dan diaktifkan akan muncul di sini."
        />
      ) : (
        <>
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Card>
              <CardBody>
                ...
                <div className="text-heading-lg tabular mt-1 font-semibold">
                  {(totalBps / 100).toLocaleString('id-ID', { maximumFractionDigits: 2 })}%
                </div>
              </CardBody>
            </Card>
          </div>
          <div className="grid gap-4">
            {holdings.map((holding) => {
              ...
              return (
                <Card key={holding.id}>
                  <CardHeader>
                    <CardTitle>{offering?.name ?? 'Offering'}</CardTitle>
                  </CardHeader>
                  <CardBody>
                    <div className="text-body-sm grid gap-4 sm:grid-cols-3">
                      ...
                      <div>
                        ...
                        <div className="tabular font-semibold">
                          {(Number(holding.ownership_bps) / 100).toLocaleString('id-ID', {
                      ...
                      <div>
                        ...
                        <div>{holding.status}</div>
                      </div>
                      <div>
                        <span className="text-fg-subtle">Akuisisi</span>
                        <div>{new Date(holding.acquisition_at).toLocaleDateString('id-ID')}</div>
                      </div>
                      <div>
                        <span className="text-fg-subtle">Transfer eligible</span>
                        <div>
