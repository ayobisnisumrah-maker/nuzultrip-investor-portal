...
export default async function InvestorProfilePage() {
  ...
  const { data: investor } = await supabase
    .from('investors')
    .select('legal_name, investor_type, country, city, address, organization_name, organization_role, whatsapp_number, bank_name, bank_account_name, bank_account_number, ktp_original_file_name, ktp_uploaded_at')
  ...
  return (
    <Stack gap={8}>
      ...
      <Card>
        ...
        <CardBody>
          <DetailList>
            ...
            <DetailRow label="Negara/Kota">{[investor?.country, investor?.city].filter(Boolean).join(' / ') || '—'}</DetailRow>
      ...
      <Card>
        <CardHeader><CardTitle>Rekening pembayaran</CardTitle></CardHeader>
        <CardBody>
          <DetailList>
            ...
            <DetailRow label="Identitas terunggah">{investor?.ktp_original_file_name ?? 'Belum diunggah'}</DetailRow>
          </DetailList>
        </CardBody>
      </Card>
    </Stack>
