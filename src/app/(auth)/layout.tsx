...
export default async function AuthLayout({ children }: { children: React.ReactNode }) {
  const [brandLogoUrl, brandName] = await Promise.all([
    getPublicBrandLogo(),
    getPublicBrandName(),
  ])
  ...
  const brand = brandLogoUrl ? (
    ...
    <>
      ...
      <span className="flex flex-col leading-tight">
        <span className="text-heading-md text-white">{brandName}</span>
      ...
      <div className={`${styles.authTheme} grid min-h-dvh lg:grid-cols-[1fr_minmax(28rem,36rem)]`}>
        <aside className="relative hidden overflow-hidden bg-black lg:flex lg:flex-col lg:justify-between lg:p-12">
          ...
          <div className="relative flex max-w-lg flex-col gap-5">
            <p className="text-white/60 overline">{brandName}</p>
            <p className="text-display-lg text-balance text-white">
              Membangun Nilai dan Kepemilikan Bersama.
            </p>
            <p className="text-body text-fg-inverse/70">
              Platform hubungan investor — informasi perusahaan, materi investor, dan pelaporan
              dalam satu tempat yang aman.
            </p>
          </div>
            ...
            >
              {brandLogoUrl ? (
                ...
                <>
                  ...
                  <span className="text-heading-sm text-fg">{brandName}</span>
                </>
              )}
            </Link>
            {children}
