...
import { replacePaymentProof } from '@/server/ownership/payment-proof-service'
...
export async function POST(request: Request) {
  ...
  if (
    ...
    !hasPermission(principal, 'profit_distribution_payments.replace_proof')
  ) {
    return NextResponse.json(
      { error: 'Anda tidak memiliki izin mengganti bukti pembayaran.' },
      { status: 403 },
    )
  }
  ...
  if (typeof allocationIdValue !== 'string' || allocationIdValue.trim().length === 0) {
    return NextResponse.json({ error: 'Allocation pembayaran wajib dipilih.' }, { status: 400 })
  }
  ...
  if (paymentReferenceValue !== null) {
    if (typeof paymentReferenceValue !== 'string') {
      return NextResponse.json({ error: 'Referensi pembayaran tidak valid.' }, { status: 400 })
    }
  ...
  try {
    ...
    const proof = await replacePaymentProof({
      supabase,
    ...
    await writeAudit(principal, {
      action: 'profit_distribution_payment_proof.replace',
      entityType: 'profit_distribution_payment_proof',
      entityId: proof.id,
      summary: 'Bukti transfer pembayaran distribusi bagi hasil berhasil diganti.',
      changes: {
        allocation_id: { before: proof.allocation_id, after: proof.allocation_id },
        original_file_name: { before: null, after: proof.original_file_name },
        mime_type: { before: null, after: proof.mime_type },
        file_size_bytes: { before: null, after: proof.file_size_bytes },
        payment_reference: { before: null, after: proof.payment_reference },
      },
    })

    return NextResponse.json({ ok: true, proof })
  } catch (error) {
    const message = error instanceof Error ? error.message : 'Gagal mengganti bukti pembayaran.'
