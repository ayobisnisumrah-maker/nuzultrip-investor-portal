...
describe('former investor historical read-only boundary', () => {
  it('introduces a read helper without broadening current_investor_id', () => {
    expect(sql).toContain('create or replace function app.current_historical_investor_id()')
  ...
  it('keeps ownership history readable for the same investor identity', () => {
    ...
    expect(sql).toContain('investor_id = app.current_historical_investor_id()')
  })

  it('keeps old message history readable while writes require active investor identity', () => {
    expect(sql).toContain('app.current_historical_investor_id() is not null')
