...
describe('financial period calendar integrity', () => {
  ...
  it('allows non-structural updates without revalidating a stored period definition', async () => {
    await db()`
      insert into public.financial_periods (
        period_type, fiscal_year, period_index, starts_on, ends_on
      ) values ('monthly', 2094, 3, date '2094-03-01', date '2094-03-31')
    `
    ...
    await expect(
      db()`
        ...
        set updated_at = now()
        where fiscal_year = 2094 and period_type = 'monthly' and period_index = 3
      `,
