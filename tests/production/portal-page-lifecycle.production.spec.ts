...
test.describe('Production Portal Page Lifecycle', () => {
  ...
  test('admin can execute Draft to Review to Approved to Published to Archived lifecycle', async ({
    ...
    await expect(
      page.getByRole('button', { name: /^Setujui$/i }),
    ).toBeVisible({
      timeout: 30_000,
    })
    ...
    await expect(
      page.getByRole('button', { name: /^Terbitkan$/i }),
    ).toBeVisible({
      timeout: 30_000,
    })
    ...
    await expect(
      page.getByRole('button', { name: /^Arsipkan$/i }),
    ).toBeVisible({
      timeout: 30_000,
    })
    ...
    await expect(
      page.locator('strong.text-fg').filter({ hasText: 'Diarsipkan' }),
    ).toBeVisible({
      timeout: 30_000,
    })
    ...
    await expect(
      page.getByText('Halaman telah diarsipkan', { exact: true }),
    ).toBeVisible({
      timeout: 30_000,
    })
